...
int LuaCuboLib::SCORE_Load(lua_State *state)
	...
	if (!cf) { LUA_SET_INT(state, 0); return 1 ; }
	if (!cf->IsHDDFile()) { LUA_SET_INT(state, 0); return 1 ; }
...
int LuaCuboLib::ARGS_Count(lua_State *state)
	{
	LUA_SET_INT(state, g_CmdLineCount());
	return 1;
	}
