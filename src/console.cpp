...
int TCuboConsole::Toggle()
{
  ...
  else { g_Game()->GetKeyboard()->SetKeyRepeatTimes(0,0);
...
for (unsigned int p=0;p<parts.size();p++)
{
 ...
 else if (toks[0]=="userlevel") { if (toks.size()>1) {
         if (g_Game()->StartLevel(toks[1],1)) was_mouse_snapped=0;
          } }
 else if (toks[0]=="level") {   if (toks.size()>1) {
         if (g_Game()->StartLevel(toks[1],0)) was_mouse_snapped=0;
          }}
 else if (toks[0]=="edit") {if (toks.size()>1)
         g_Game()->StartLevel(toks[1],2);
         else
         if (g_Game()->StartLevel("",2)) was_mouse_snapped=1;
          }
 else if (toks[0]=="bind") {
...
else if (ident2==SDLK_UP) { if (hisindex==0) hisbackup=history.back();
                           hisindex++;
...
void TCuboConsole::Render()
{
    ...
    if (isactive==false)  { lowerline_ypos+=0.2;
                      if (lowerline_ypos>0.5) { lowerline_ypos=0.5; return ; }
                    }
    else { lowerline_ypos-=0.2; if (lowerline_ypos<-0.2) lowerline_ypos=-0.2;
